...
//    });
    ...
    // dipanggil di ijinhaki
    $app->get('/ijinhaki/selesai/{id}', function($request, $response, $args) {
    	$id = $args['id'];
    	$update = ExecuteUpdate("UPDATE ijinhaki ih, brand b SET ih.selesai=1, ih.readonly=1, ih.`status`='Selesai', b.ijinhaki=1 WHERE ih.idbrand = b.id AND ih.id=".$id);
    	return $response->withJson(1);
    });
    $app->get('/ijinhaki/belumselesai/{id}', function($request, $response, $args) {
    	$id = $args['id'];
    	$update = ExecuteUpdate("UPDATE ijinhaki ih, brand b SET ih.selesai=0, b.ijinhaki=-1 WHERE ih.idbrand = b.id AND ih.id=".$id);
    	updateStatus("ijinhaki", $id);
    	return $response->withJson(1);
    });
