...
loadjs.ready("head", function () {
    ...
    fnpd_extendadd = currentForm = new ew.Form("fnpd_extendadd", "add");
    ...
    // Validate form
    fnpd_extendadd.validate = function () {
    ...
    // Form_CustomValidate
    fnpd_extendadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fnpd_extendadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fnpd_extendadd.lists.readonly = <?= $Page->readonly->toClientList($Page) ?>;
    loadjs.done("fnpd_extendadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fnpd_extendadd" id="fnpd_extendadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fnpd_extendadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fnpd_extendadd", "x_tglbayar", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fnpd_extendadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fnpd_extendadd", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->readonly->isInvalidClass() ?>" data-table="npd_extend" data-field="x_readonly" name="x_readonly[]" id="x_readonly_907715" value="1"<?= ConvertToBool($Page->readonly->CurrentValue) ? " checked" : "" ?><?= $Page->readonly->editAttributes() ?> aria-describedby="x_readonly_help">
    <label class="custom-control-label" for="x_readonly_907715"></label>
</div>
