...
<table id="tbl_v_piutangmaster" class="table ew-view-table ew-master-table ew-vertical">
    <tbody>
<?php if ($v_piutang->idpegawai->Visible) { // idpegawai ?>
        <tr id="r_idpegawai">
            <td class="<?= $v_piutang->TableLeftColumnClass ?>"><?= $v_piutang->idpegawai->caption() ?></td>
            <td <?= $v_piutang->idpegawai->cellAttributes() ?>>
<span id="el_v_piutang_idpegawai">
<span<?= $v_piutang->idpegawai->viewAttributes() ?>>
<?= $v_piutang->idpegawai->getViewValue() ?></span>
</span>
</td>
        </tr>
<?php } ?>
