...
class Brand extends DbTable
{
    ...
    public function applyUserIDFilters($filter)
    {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            if ($this->getCurrentMasterTable() == "customer" || $this->getCurrentMasterTable() == "") {
                $filter = $this->addDetailUserIDFilter($filter, "customer"); // Add detail User ID filter
            }
        }
        return $filter;
    }
...
SORTHTML;
    ...
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "customer") {
            $filterWrk = Container("customer")->addUserIDFilter($filterWrk);
        }
        return $filterWrk;
    }
    ...
    public function addDetailUserIDFilter($filter, $currentMasterTable)
    {
        $filterWrk = $filter;
        if ($currentMasterTable == "customer") {
            $mastertable = Container("customer");
            if (!$mastertable->userIdAllow()) {
                $subqueryWrk = $mastertable->getUserIDSubquery($this->idcustomer, $mastertable->id);
