 ...
 * Page class
 */
class NpdExtendAdd extends NpdExtend
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NpdExtendAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("NpdExtendList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "NpdExtendList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "NpdExtendView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->buktipembayaran->CurrentValue = null;
        $this->buktipembayaran->OldValue = $this->buktipembayaran->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->tglbayar->IsDetailKey) {
            ...
            $this->tglbayar->CurrentValue = UnFormatDateTime($this->tglbayar->CurrentValue, 0);
        }

        // Check field name 'buktipembayaran' first before field var 'x_buktipembayaran'
        $val = $CurrentForm->hasValue("buktipembayaran") ? $CurrentForm->getValue("buktipembayaran") : $CurrentForm->getValue("x_buktipembayaran");
        if (!$this->buktipembayaran->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->buktipembayaran->Visible = false; // Disable update for API request
            } else {
                $this->buktipembayaran->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->tglbayar->CurrentValue = UnFormatDateTime($this->tglbayar->CurrentValue, 0);
        $this->buktipembayaran->CurrentValue = $this->buktipembayaran->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['buktipembayaran'] = $this->buktipembayaran->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->tglbayar->EditValue = HtmlEncode(FormatDateTime($this->tglbayar->CurrentValue, 8));
            ...
            $this->buktipembayaran->EditValue = HtmlEncode($this->buktipembayaran->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->tglbayar->Required) {
            if (!$this->tglbayar->IsDetailKey && EmptyValue($this->tglbayar->FormValue)) {
                $this->tglbayar->addErrorMessage(str_replace("%s", $this->tglbayar->caption(), $this->tglbayar->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->tglbayar->FormValue)) {
            $this->tglbayar->addErrorMessage($this->tglbayar->getErrorMessage(false));
        }
        if ($this->buktipembayaran->Required) {
            if (!$this->buktipembayaran->IsDetailKey && EmptyValue($this->buktipembayaran->FormValue)) {
                $this->buktipembayaran->addErrorMessage(str_replace("%s", $this->buktipembayaran->caption(), $this->buktipembayaran->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // tglbayar
        $this->tglbayar->setDbValueDef($rsnew, UnFormatDateTime($this->tglbayar->CurrentValue, 0), CurrentDate(), false);

        // buktipembayaran
        $this->buktipembayaran->setDbValueDef($rsnew, $this->buktipembayaran->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("NpdExtendList"), "", $this->TableVar, true);
