...
class Ijinhaki extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // buktibayar
        $this->buktibayar = new DbField('ijinhaki', 'ijinhaki', 'x_buktibayar', 'buktibayar', '`buktibayar`', '`buktibayar`', 200, 255, -1, true, '`buktibayar`', false, false, false, 'IMAGE', 'FILE');
        $this->buktibayar->Nullable = false; // NOT NULL field
        $this->buktibayar->Required = true; // Required field
        $this->buktibayar->Sortable = true; // Allow sort
        $this->buktibayar->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->buktibayar->Param, "CustomMsg");
    ...
    public function deleteUploadedFiles($row)
    {
        ...
        $oldFiles = EmptyValue($row['buktibayar']) ? [] : [$row['buktibayar']];
        foreach ($oldFiles as $oldFile) {
            if (file_exists($this->buktibayar->oldPhysicalUploadPath() . $oldFile)) {
...
SORTHTML;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->buktibayar);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->buktibayar);
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        $fileNameFld = "";
        $fileTypeFld = "";
        if ($fldparm == 'buktibayar') {
            $fldName = "buktibayar";
            $fileNameFld = "buktibayar";
        } else {
