...
class IjinhakiAdd extends Ijinhaki
{
    ...
    protected function getUploadFiles()
    {
        ...
        $this->buktibayar->Upload->Index = $CurrentForm->Index;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->buktibayar->setDbValue($this->buktibayar->Upload->DbValue);
    ...
    protected function newRow()
    {
        ...
        $row['buktibayar'] = $this->buktibayar->Upload->DbValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // buktibayar
            if (!EmptyValue($this->buktibayar->Upload->DbValue)) {
                $this->buktibayar->ImageAlt = $this->buktibayar->alt();
            ...
            if (!EmptyValue($this->buktibayar->Upload->DbValue)) {
                $this->buktibayar->HrefValue = GetFileUploadUrl($this->buktibayar, $this->buktibayar->htmlDecode($this->buktibayar->Upload->DbValue)); // Add prefix/suffix
                $this->buktibayar->LinkAttrs["target"] = ""; // Add target
                if ($this->isExport()) {
                    $this->buktibayar->HrefValue = FullUrl($this->buktibayar->HrefValue, "href");
                }
            } else {
                $this->buktibayar->HrefValue = "";
            }
            $this->buktibayar->ExportHrefValue = $this->buktibayar->UploadPath . $this->buktibayar->Upload->DbValue;
            $this->buktibayar->TooltipValue = "";
            if ($this->buktibayar->UseColorbox) {
                if (EmptyValue($this->buktibayar->TooltipValue)) {
                    $this->buktibayar->LinkAttrs["title"] = $Language->phrase("ViewImageGallery");
                }
                $this->buktibayar->LinkAttrs["data-rel"] = "ijinhaki_x_buktibayar";
                $this->buktibayar->LinkAttrs->appendClass("ew-lightbox");
            }
            ...
            if (!EmptyValue($this->buktibayar->CurrentValue)) {
                $this->buktibayar->Upload->FileName = $this->buktibayar->CurrentValue;
            }
            if ($this->isShow() || $this->isCopy()) {
                RenderUploadField($this->buktibayar);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->buktibayar->Required) {
            if ($this->buktibayar->Upload->FileName == "" && !$this->buktibayar->Upload->KeepFile) {
                $this->buktibayar->addErrorMessage(str_replace("%s", $this->buktibayar->caption(), $this->buktibayar->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->buktibayar->Visible && !$this->buktibayar->Upload->KeepFile) {
            $oldFiles = EmptyValue($this->buktibayar->Upload->DbValue) ? [] : [$this->buktibayar->htmlDecode($this->buktibayar->Upload->DbValue)];
            if (!EmptyValue($this->buktibayar->Upload->FileName)) {
                $newFiles = [$this->buktibayar->Upload->FileName];
                $NewFileCount = count($newFiles);
                for ($i = 0; $i < $NewFileCount; $i++) {
                    if ($newFiles[$i] != "") {
                        $file = $newFiles[$i];
                        $tempPath = UploadTempPath($this->buktibayar, $this->buktibayar->Upload->Index);
                        if (file_exists($tempPath . $file)) {
                            ...
                            $file1 = UniqueFilename($this->buktibayar->physicalUploadPath(), $file); // Get new file name
                            if ($file1 != $file) { // Rename temp file
                                while (file_exists($tempPath . $file1) || file_exists($this->buktibayar->physicalUploadPath() . $file1)) { // Make sure no file name clash
                                    $file1 = UniqueFilename([$this->buktibayar->physicalUploadPath(), $tempPath], $file1, true); // Use indexed name
                                }
                ...
                $this->buktibayar->Upload->DbValue = empty($oldFiles) ? "" : implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $oldFiles);
                $this->buktibayar->Upload->FileName = implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $newFiles);
                $this->buktibayar->setDbValueDef($rsnew, $this->buktibayar->Upload->FileName, "", false);
            }
        }
        ...
        if ($insertRow) {
            ...
            if ($addRow) {
                if ($this->buktibayar->Visible && !$this->buktibayar->Upload->KeepFile) {
                    ...
                    if (!EmptyValue($this->buktibayar->Upload->FileName)) {
                        $newFiles = [$this->buktibayar->Upload->FileName];
                        $newFiles2 = [$this->buktibayar->htmlDecode($rsnew['buktibayar'])];
                        $newFileCount = count($newFiles);
                        for ($i = 0; $i < $newFileCount; $i++) {
                            if ($newFiles[$i] != "") {
                                $file = UploadTempPath($this->buktibayar, $this->buktibayar->Upload->Index) . $newFiles[$i];
                                if (file_exists($file)) {
                                    if (@$newFiles2[$i] != "") { // Use correct file name
                                        $newFiles[$i] = $newFiles2[$i];
                                    }
                                    if (!$this->buktibayar->Upload->SaveToFile($newFiles[$i], true, $i)) { // Just replace
                    ...
                    if (Config("DELETE_UPLOADED_FILES")) {
                        foreach ($oldFiles as $oldFile) {
                            if ($oldFile != "" && !in_array($oldFile, $newFiles)) {
                                @unlink($this->buktibayar->oldPhysicalUploadPath() . $oldFile);
                            }
                        }
                    }
                }
        ...
        if ($addRow) {
            // buktibayar
            CleanUploadTempPath($this->buktibayar, $this->buktibayar->Upload->Index);
        }
    ...
    public function pageDataRendering(&$header)
    {
        // Example:
        //$header = "your header";
        $this->idcustomer->CustomMsg = "Pastikan Customer telah melengkapi data KTP & NPWP.";
        $this->idbrand->CustomMsg = "Pastikan brand telah mengunggah logo & akta perusahaan.";
        $this->buktibayar->CustomMsg = "Biaya: Rp. 3.500.000,00";
    }
    ...
    public function formCustomValidate(&$customError)
    {
        ...
        $dataCustomer = ExecuteRow("SELECT c.ktp, c.npwp, b.aktaperusahaan, b.logo FROM customer c, brand b WHERE c.id=b.idcustomer AND b.id=".$this->idbrand->FormValue);
        if (empty($dataCustomer['ktp']) || empty($dataCustomer['npwp'])) {
        	$customError = "Customer belum melengkapi data KTP atau NPWP.";
        	return false;
        }
        if (empty($dataCustomer['aktaperusahaan']) || empty($dataCustomer['logo'])) {
        	$customError = "Brand belum mengunggah logo atau akta perusahaan.";
        	return false;
        }
        return true;
    }
