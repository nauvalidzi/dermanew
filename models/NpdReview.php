...
class NpdReview extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // id
        $this->id = new DbField('npd_review', 'npd_review', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // idnpd
        $this->idnpd = new DbField('npd_review', 'npd_review', 'x_idnpd', 'idnpd', '`idnpd`', '`idnpd`', 3, 11, -1, false, '`idnpd`', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        ...
        $this->wadah->Nullable = false; // NOT NULL field
        $this->wadah->Required = true; // Required field
        ...
        // kesimpulan
        $this->kesimpulan = new DbField('npd_review', 'npd_review', 'x_kesimpulan', 'kesimpulan', '`kesimpulan`', '`kesimpulan`', 200, 255, -1, false, '`kesimpulan`', false, false, false, 'FORMATTED TEXT', 'TEXT');
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->kesimpulan->Raw) {
            $this->kesimpulan->CurrentValue = HtmlDecode($this->kesimpulan->CurrentValue);
        }
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        $myResult = ExecuteUpdate("UPDATE npd_sample SET `status`=".$rsnew['status'].", readonly=1 WHERE id=".$rsnew['idnpd_sample']);
        return true;
    }
    ...
    public function rowUpdating($rsold, &$rsnew)
    {
        // Enter your code here
        // To cancel, set return value to false
        $myResult = ExecuteUpdate("UPDATE npd_sample SET `status`=".$rsnew['status']." WHERE id=".$rsold['idnpd_sample']);
        return true;
    }
