...
class NpdConfirmPreview extends NpdConfirm
{
    ...
    public function run()
    {
        ...
        $this->tglkonfirmasi->Visible = false;
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->CurrentOrder !== "") {
            ...
            $this->updateSort($this->namapemesan, $ctrl); // namapemesan
            $this->updateSort($this->personincharge, $ctrl); // personincharge
            $this->updateSort($this->notelp, $ctrl); // notelp
        }
    }
            ...
            switch ($fld->FieldVar) {
                case "x_idnpd_sample":
                    $lookupFilter = function () {
                        return CurrentPageID() == "add" ? "id IN (SELECT idnpd_sample FROM npd_review WHERE `status`=1 AND readonly=0 AND idnpd = ".$this->idnpd->CurrentValue.")" : "";
                    };
