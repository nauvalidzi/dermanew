...
class NpdHargaAdd extends NpdHarga
{
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->idviskositasbarang->CurrentValue = null;
        $this->idviskositasbarang->OldValue = $this->idviskositasbarang->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idviskositasbarang' first before field var 'x_idviskositasbarang'
        $val = $CurrentForm->hasValue("idviskositasbarang") ? $CurrentForm->getValue("idviskositasbarang") : $CurrentForm->getValue("x_idviskositasbarang");
        if (!$this->idviskositasbarang->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->idviskositasbarang->Visible = false; // Disable update for API request
            } else {
                $this->idviskositasbarang->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->idviskositasbarang->CurrentValue = $this->idviskositasbarang->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['idviskositasbarang'] = $this->idviskositasbarang->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // idviskositasbarang
            $this->idviskositasbarang->EditAttrs["class"] = "form-control";
            $this->idviskositasbarang->EditCustomAttributes = "";
            $curVal = trim(strval($this->idviskositasbarang->CurrentValue));
            if ($curVal != "") {
                $this->idviskositasbarang->ViewValue = $this->idviskositasbarang->lookupCacheOption($curVal);
            } else {
                $this->idviskositasbarang->ViewValue = $this->idviskositasbarang->Lookup !== null && is_array($this->idviskositasbarang->Lookup->Options) ? $curVal : null;
            }
            if ($this->idviskositasbarang->ViewValue !== null) { // Load from cache
                $this->idviskositasbarang->EditValue = array_values($this->idviskositasbarang->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->idviskositasbarang->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->idviskositasbarang->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->idviskositasbarang->EditValue = $arwrk;
            }
            $this->idviskositasbarang->PlaceHolder = RemoveHtml($this->idviskositasbarang->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->idviskositasbarang->Required) {
            if (!$this->idviskositasbarang->IsDetailKey && EmptyValue($this->idviskositasbarang->FormValue)) {
                $this->idviskositasbarang->addErrorMessage(str_replace("%s", $this->idviskositasbarang->caption(), $this->idviskositasbarang->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // idviskositasbarang
        $this->idviskositasbarang->setDbValueDef($rsnew, $this->idviskositasbarang->CurrentValue, 0, false);
        ...
        // bariskarton
        $this->bariskarton->setDbValueDef($rsnew, $this->bariskarton->CurrentValue, 0, false);

        // kolomkarton
        $this->kolomkarton->setDbValueDef($rsnew, $this->kolomkarton->CurrentValue, 0, false);

        // stackkarton
        $this->stackkarton->setDbValueDef($rsnew, $this->stackkarton->CurrentValue, 0, false);

        // isikarton
        $this->isikarton->setDbValueDef($rsnew, $this->isikarton->CurrentValue, 0, false);
        ...
        // orderperdana
        $this->orderperdana->setDbValueDef($rsnew, $this->orderperdana->CurrentValue, 0, false);

        // orderkontrak
        $this->orderkontrak->setDbValueDef($rsnew, $this->orderkontrak->CurrentValue, 0, false);
